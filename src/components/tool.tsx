...
import React, { useState } from 'react'; import Image from 'next/image'; import { motion } from 'framer-motion';

// Define all types

type ComponentSummaryItem = { type: string; count: number; };

type ICAssignment = { [label: string]: string; };

type WireCount = { total_circuit_connections: number; total_power_connections: number; overall_total: number; };
...
const handleFileInputChange = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file && file.type.startsWith('image/')) { setImage(file); setPreviewUrl(URL.createObjectURL(file)); } };

const triggerFileSelect = () => document.getElementById('fileInput')?.click();

const handleRemoveImage = () => { setImage(null); setPreviewUrl(null); setAnalysisResult(null); setError(null); };
